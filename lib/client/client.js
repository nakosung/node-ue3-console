(function() {
  var app;

  app = angular.module('node', ['ui', 'ui.bootstrap']);

  app.directive('xychart', function() {
    var height, margin, width;

    margin = 20;
    width = 200;
    height = 200 - .5 - margin;
    return {
      restrict: 'E',
      terminal: true,
      scope: {
        val: '='
      },
      link: function(scope, element, attrs) {
        var vis, watchBody;

        vis = d3.select(element[0]).append('svg').attr('width', width).attr('height', height + margin + 100);
        watchBody = function(newVal, oldVal) {
          var line, x, xx, y, yy;

          vis.selectAll('*').remove();
          xx = _.pluck(newVal, 'x').map(parseFloat);
          yy = _.pluck(newVal, 'y').map(parseFloat);
          x = d3.scale.linear().domain([d3.min(xx), d3.max(xx)]).range([0, width]);
          y = d3.scale.linear().domain([d3.min(yy), d3.max(yy)]).range([height, 0]);
          line = d3.svg.line().x(function(d) {
            return parseFloat(x(d.x));
          }).y(function(d) {
            return parseFloat(y(d.y));
          });
          return vis.append('svg:path').attr('d', line(newVal));
        };
        return scope.$watch('val', watchBody, true);
      }
    };
  });

  app.directive('ngBlur', [
    '$parse', function($parse) {
      return function(scope, element, attr) {
        var fn;

        fn = $parse(attr['ngBlur']);
        return element.bind('blur', function(event) {
          return scope.$apply(function() {
            return fn(scope, {
              $event: event
            });
          });
        });
      };
    }
  ]);

  app.directive('showonhoverparent', function() {
    return {
      link: function(scope, element, attrs) {
        element.parent().bind('mouseenter', function() {
          return element.show();
        });
        element.parent().bind('mouseleave', function() {
          return element.hide();
        });
        return element.hide();
      }
    };
  });

  app.factory('node', function($rootScope) {
    var Server;

    Server = (function() {
      function Server() {
        this.plot = [];
        this.watches = {};
        this.code = "log 'Hello world'";
        this.logs = "Connecting...\n";
        this.handleConnectionLost();
      }

      Server.prototype.updateAngularJs = function() {
        return !$rootScope.$$phase && $rootScope.$apply();
      };

      Server.prototype.handleConnectionLost = function() {
        var _this = this;

        this.list = null;
        this.active = null;
        this.online = false;
        this.print("Trying to connect");
        this.sock = new SockJS('/echo');
        this.sock.onopen = function() {
          return _this.initSock();
        };
        return this.sock.onclose = function() {
          return setTimeout((function() {
            return _this.handleConnectionLost();
          }), 250);
        };
      };

      Server.prototype.initSock = function() {
        var _this = this;

        this.online = true;
        this.print('Connected to node.js');
        this.send({
          list: null,
          hosts: null,
          watch: 'pc.Pawn.Location'
        });
        return this.sock.onmessage = function(e) {
          var data, exception, k, o, plotted, v;

          try {
            data = JSON.parse(e.data);
            for (k in data) {
              v = data[k];
              switch (k) {
                case "refresh":
                  _this.refresh();
                  break;
                case "log":
                  plotted = false;
                  try {
                    o = JSON.parse(v);
                    if (o.plot) {
                      _this.plot.push(o.plot);
                      plotted = true;
                    }
                  } catch (_error) {
                    e = _error;
                  }
                  if (!plotted) {
                    _this.print(v);
                  }
                  break;
                case "code":
                case "title":
                case "active":
                case "list":
                case "hosts":
                case "host":
                case "running":
                  _this[k] = v;
                  break;
                case "watch":
                  _this.watches[v.key] = v.val;
                  break;
                case "invalidated":
                  _this.send({
                    list: null
                  });
              }
            }
          } catch (_error) {
            exception = _error;
            console.log(exception, e);
          }
          return _this.updateAngularJs();
        };
      };

      Server.prototype.refresh = function() {
        return window.location.reload();
      };

      Server.prototype.send = function(json) {
        return this.sock.send(JSON.stringify(json));
      };

      Server.prototype.load = function(target) {
        return this.send({
          load: target
        });
      };

      Server.prototype.discard = function(target) {
        return this.send({
          discard: target
        });
      };

      Server.prototype.save = function() {
        return this.send({
          save: {
            code: this.code,
            title: this.title
          }
        });
      };

      Server.prototype.switchHost = function(host) {
        return this.send({
          host: host
        });
      };

      Server.prototype.run = function(opts) {
        this.plot = [];
        this.updateAngularJs();
        if (opts != null ? opts.stop : void 0) {
          return this.send({
            stop: true
          });
        } else {
          return this.send({
            run: this.code
          });
        }
      };

      Server.prototype.print = function(msg) {
        console.log(msg);
        this.logs += msg + "\n";
        return this.updateAngularJs();
      };

      Server.prototype.watch = function(key) {
        this.send({
          watch: key
        });
        this.watches[key] = '..pending..';
        return this.updateAngularJs();
      };

      Server.prototype.unwatch = function(key) {
        delete this.watches[key];
        this.send({
          unwatch: key
        });
        return this.updateAngularJs();
      };

      return Server;

    })();
    return new Server();
  });

  app.controller('CodeCtrl', function($scope, node) {
    $scope.server = node;
    $scope.status = function() {
      if (node.online) {
        return "ONLINE";
      } else {
        return "OFFLINE";
      }
    };
    $scope.addWatch = function(val) {
      node.watch(val);
      return $scope.cancelEdit();
    };
    $scope.edit = function(key, value) {
      $scope.edit_target = key;
      return $scope.edit_value = value;
    };
    $scope.save = function(key, value) {
      node.send({
        run: "" + key + " = " + value
      });
      return $scope.edit_target = null;
    };
    $scope.cancelEdit = function() {
      return $scope.edit_target = null;
    };
    $scope.run_action = function() {
      if (node.running) {
        return "Stop";
      } else {
        return "Run";
      }
    };
    return $(document).keydown(function(e) {
      if (e.keyCode === 66 && e.ctrlKey) {
        node.run();
        false;
      }
      if (e.keyCode === 83 && e.ctrlKey) {
        node.save();
        return false;
      }
    });
  });

}).call(this);
