(function() {
  var Rotator, UEPrim, Vector3, async, from_unrealengine3, to_unrealengine3, _,
  ...
  from_unrealengine3 = function(text, bridge, cb) {
    var array, elems, num, obj, rot, seq, vec3;
    ...
    if (vec3) {
      ...
      if (rot) {
        ...
        if (array) {
          elems = array[1].split(',');
          console.log(elems);
          seq = elems.map(function(e) {
            return function(cb) {
              return from_unrealengine3(e, bridge, function(result) {
                return cb(null, result);
              });
            };
          });
          return async.parallel(seq, function(err, result) {
            return cb(result);
          });
        } else {
